...
1. **NEVER** 跳过 Phase 0 弹窗 —— feature 命名、输出位置、执行模式必须由用户敲定
...
收到大型需求后立即弹 `AskUserQuestion`，问 4 件事：
...
| 执行模式 | 写完文档后怎么办？| (A) 停下，开新 AI 窗口执行（推荐）(B) 直接执行 (C) 写完先停 |
