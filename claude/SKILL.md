---
name: ultra-plan
description: Use when user describes a large-scale architecture or multi-module feature that needs 5-15 parallel research agents and produces a folder of planning markdowns instead of a single plan file. Triggers on phrases like "ultra-plan", "/ultra-plan", "large architecture", "整套架构", "整个系统", "全套", or any request that visibly spans 5+ modules / crosses frontend+backend / requires multiple AI sessions to execute. Skip for single-file fixes, single-module features, bug fixes, or routine refactors that fit a normal /plan.
---
...
普通 `/plan` 输出**一个** plan 文件，对单功能足够。但当用户输入「整套管理端」「全套支付系统」「跨前后端的 IM 富媒体」这种**5+ 模块、跨前后端、需要多 AI session 分头执行**的需求时，单个 plan 颗粒度不够，开发者会在执行时迷失。

ultra-plan 把需求**拆成一组结构化 md**：1 个 `00_总体规划.md`（全局视图 + 模块依赖图 + 派发指令）+ N 个模块 md（每个模块独立的 7 节实现指引）。开发者可以拿着 00_总体规划.md 喂给一个新 AI session，让它按图索骥派 subagent 执行各模块。

**项目无关**：本 skill 不内嵌任何项目特定约定。项目自身的约定（API 风格、env 路径、测试标准、目录布局等）由 ultra-plan 在 Phase 0 自动**读取项目级配置文件**（`CLAUDE.md` / `AGENTS.md` / `.claude/CLAUDE.md` / `.cursorrules` 等）后注入到生成的 md 中。
...
|---|---|---|
| 执行模式 | 写完所有 md 后怎么办？| (A) 推荐：停下，开新 AI 窗口让它读 `00_总体规划.md` 执行（避免主 session 上下文爆）(B) 直接接着派 subagent 跑 (C) 写完先停，我自己看了再决定 |
...
- `CLAUDE.md` / `.claude/CLAUDE.md`（Claude Code 项目指令）
...
立即弹 `AskUserQuestion`，问 3 件事（**已不含执行模式 —— 那个在 0.0 已经定了**）：

| Header | Question | Options |
|---|---|---|
| feature 命名 | 这次需求的 feature 名是？（决定目录名）| 主 agent 提议 2-3 个候选 + Other 让用户敲 |
...
主 agent 综合需求 + Phase 0 答案后，**单条消息派 5-15 个 Explore subagent 并行调研**。
...
**只对低耦合模块**用 `general-purpose` subagent 并行写：
...
每个模块 md 严格遵循 7 节框架（模板见 [templates/模块.md.template](templates/模块.md.template)）：
...
3. **按 Phase 0.0 的答案直接流转，禁止再弹窗问执行模式**：

   - 0.0 = (A) 新窗口 → 输出：「✓ 准备好了。请开新 AI 窗口，第一句喂它：『读 `<output_dir>/00_总体规划.md` 后按计划执行』」然后停下
   - 0.0 = (B) 直接执行 → 立即进入 Phase 5
   - 0.0 = (C) 写完先停 → 输出文档清单，停下

**禁止在 Phase 4 重新弹窗问执行模式** —— 0.0 已经定了，再问就是流程冗余。
...
| "Phase 1 派 3 个 subagent 就够了" | NO. < 5 个就用普通 /plan，不要用 ultra-plan |
...
| "项目没 CLAUDE.md，我就用其他项目的约定" | NO. 项目无 CLAUDE.md = 不注入项目红线段，纯通用模板。不要从训练数据 / 其他项目带入约定 |
