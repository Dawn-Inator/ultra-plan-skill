...
## §5. `request_user_input` 模板（Phase 0 + Phase 4）

### §5.1 Phase 0: 范围澄清

Codex 的 `request_user_input` 一次最多 3 个问题，Phase 0 分两次问：第一轮锁定 feature / 输出位置 / 执行模式，第二轮锁定调研深度。不要包含多选字段，客户端会自动提供 free-form Other。
  ...
  questions: [
    {
      ...
      options: [
        { label: "新窗口执行 (Recommended)",
          description: "把 <output_dir>/00_总体规划.md 路径丢给新 Codex session，让它执行。当前 session 保持轻量" },
        { label: "直接执行",
