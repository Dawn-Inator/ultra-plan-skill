...
```
你是 ultra-plan Phase 1 调研 subagent，agent_type=explorer。
...
- 项目根：<project_root>
- 项目约定：<project_root>/AGENTS.md（如存在，优先必读）
- 项目约定补充：<project_root>/CLAUDE.md / .cursorrules（如存在）
- 项目 memory（如相关）：<具体路径>

## 输出要求（不限字数，详细度优先）

主 agent 会用你的报告做模块 md 的素材源——**素材稀薄 = 模块 md 没料**。按以下结构按需充分展开：

1. **现状摘要**：相关文件 / 函数 / 表 / 端点全部列出，每条**必须带绝对路径或 file:line**
2. **可复用的现有实现**：尽你所能列出可复用点（3 个起，上不封顶），每条带 file:line + 一句"为什么可复用"
3. **潜在风险点**：与新需求冲突的现有约定 / 历史 incident / 隐式契约（每条带证据 file:line）
4. **推荐做法**：基于现状给具体建议（含落地路径 / 函数签名 / 表结构等），禁止泛泛"建议设计良好"
...
- **不要刻意精简**——主 agent 上下文够大（Codex GPT-5 200k），素材完整比节约 token 重要得多
```
...
```
你是 ultra-plan Phase 3 写文档 subagent，agent_type=worker。
...
主权威（必须复述路径作为开头）：
  ...
  2. ~/.codex/skills/ultra-plan/templates/模块.md.template

上下文：
  - <project_root>/AGENTS.md（如存在，优先）
  - <project_root>/CLAUDE.md（如存在）
  - ~/.codex/skills/ultra-plan/references/项目适配指南.md
...
## 详细度要求（最高优先级，对应 SKILL.md Hard rule #10）

模块 md 是另起 AI session 的执行者的**唯一权威源**——他看不到 00_总体规划.md 之外的上下文。所以你写的每节都要展开到"零参考可执行"：

- §2 现状定位：列全相关 file:line，不只是 3-5 个示例
- §3 实现指引：给完整目录结构 + 函数签名（含返回类型 + docstring 一句话）+ 关键算法伪代码
- §4 API 契约：URL / 请求字段（每个字段给类型 + 是否必填 + 一句解释）/ 响应字段（同上）/ 错误码全列
- §6 验收红线：测试场景具体到"用户输入 X → 期望响应 Y"，不要 "happy path 跑通"
- §7 边界 & TBD：所有产品决策项列清，附**默认推荐答案**（让执行者敢自己拍板）

**不限字数**。模块 md 越详细，执行者越不需要回头问。
...
```
你是 ultra-plan Phase 5 实现 subagent，agent_type=worker。
...
- 写代码时遵循项目级 AGENTS.md / CLAUDE.md 全部约定，冲突时按当前会话的指令优先级处理
- 写完跑 §6 验收红线里的对应测试
...
```
你是 ultra-plan Phase 5 spec reviewer subagent，agent_type=worker。
...
## §5. `request_user_input` 模板（Phase 0）

Codex 的 `request_user_input` 一次最多 3 个问题。ultra-plan 固定分两次问：Phase 0.0 先单独锁定执行模式；Phase 0.1 读项目配置；Phase 0.2 再一次锁定 feature / 输出位置 / 调研深度。

如果当前不是 Codex Plan Mode，或 `request_user_input` 不可用，不要调用该工具；用普通消息让用户切到 Plan Mode，或让用户直接回复对应选项。
  ...
  questions: [
    {
      ...
      header: "执行模式",
      options: [
        { label: "新窗口执行 (Recommended)",
          description: "大型任务建议另起 session，避免主 session 上下文爆。开发者复制 00_总体规划.md 路径丢给新 Codex session 即可" },
        { label: "直接执行",
...
```typescript
{
  questions: [
    ...
    },
    {
      ...
      header: "输出位置",
      options: [
        { label: ".todolist (Recommended)",
          description: "放到 <project_root>/.todolist/<feature>-v<N>/，常见且适合临时规划。" },
        { label: "docs/plans",
          description: "若项目用 docs/ 集中放规划" },
        { label: "现有目录",
          description: "先扫描现有规划目录，再沿用最接近的路径。" }
      ]
    },
    {
      id: "research_depth",
...
Phase 4 禁止再次调用 `request_user_input` 问执行模式。写完所有 md 后，按 Phase 0.0 的 `execution_mode` 直接分支：
